...
| --- | --- |
| 共同主办 | 加拿大、墨西哥、美国 |
| 亚足联 | 澳大利亚、伊拉克、伊朗、日本、约旦、韩国、卡塔尔、沙特阿拉伯、乌兹别克斯坦 |
| 非洲足联 | 阿尔及利亚、佛得角、刚果民主共和国、科特迪瓦、埃及、加纳、摩洛哥、塞内加尔、南非、突尼斯 |
| 中北美及加勒比足联 | 库拉索、海地、巴拿马 |
| 南美足联 | 阿根廷、巴西、哥伦比亚、厄瓜多尔、巴拉圭、乌拉圭 |
| 大洋洲足联 | 新西兰 |
| 欧足联 | 奥地利、比利时、波黑、克罗地亚、捷克、英格兰、法国、德国、荷兰、挪威、葡萄牙、苏格兰、西班牙、瑞典、瑞士、土耳其 |
...
6. 任何缺失字段写为 `null`，并把 `coverageStatus` 标为 `缺少公开来源` 或 `需授权数据源`，不得用模型猜测补值。

## 字段采集矩阵

| 数据域 | 字段 | 推荐来源 | 覆盖状态 |
| --- | --- | --- | --- |
| 国家队近期比赛 | 最近 2 年正式比赛、友谊赛、世界杯预选赛、洲际杯赛、欧国联、美洲杯、亚洲杯、非洲杯、中北美金杯赛 | FIFA、各洲足联、国家足协、Sportradar 足球 API | 公开核心数据 |
| 国家队比赛技术统计 | 比分、控球率、射门数、射正数、传球成功率、红黄牌 | FIFA Match Centre、各洲足联、Sportradar Soccer API | public_partial |
| 国家队高级比赛统计 | xG、xGA、禁区触球、定位球进攻、防守拦截、失误导致射门、对手强度 | StatsBomb、Opta/Stats Perform、Wyscout、Sportradar 高级包 | 需授权数据源 |
| 球队 xG/xGA | 场均 xG、场均 xGA、非点球 xG、定位球 xG、反击 xG、开放战 xG、被对手创造的大机会、xG 差值 | StatsBomb、Opta/Stats Perform、Wyscout、Sportradar 高级包 | 需授权数据源 |
...
| 阵容稳定性 | 最近 10 场首发重复率、核心共同出场、中卫组合、中场三人组、锋线组合、门将固定程度、平均国家队出场、平均年龄 | FIFA/足协比赛阵容、Sportradar 阵容数据 | 公开核心数据 |
...
| 地点天气旅行 | 城市、球场、草皮、温度、湿度、海拔、当地时间、上一场地点、飞行距离、休息天数、跨时区 | FIFA 赛程、球场信息、天气 API、地理距离计算 | 公开核心数据 |
