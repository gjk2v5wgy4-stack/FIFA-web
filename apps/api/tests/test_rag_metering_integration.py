...
def test_approved_user_with_enough_tokens_can_call_rag_and_writes_metering_logs() -> None:
    ...
    assert data["usage"]["tokensCharged"] == 640
    assert data["usage"]["remainingTokens"] == 860
    ...
    assert [entry.amount_tokens for entry in ledger] == [1500, -640]
    ...
    assert usage_logs[0].internal_tokens_charged == 640
...
def test_rag_idempotency_key_prevents_duplicate_token_charge() -> None:
    ...
    assert app.state.services.tokens.getBalance("user_approved") == 1000
